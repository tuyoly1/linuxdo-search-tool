...
function renderCard(card) {
  ...
  const primaryLink = card.links.find((link) => link.primary) || card.links[0];
  const backupLinks = card.links.filter((link) => link !== primaryLink);
  const visibleLinks = [
    primaryLink,
    ...backupLinks.filter((link) => link.label.includes("Bing")).slice(0, 1),
    ...backupLinks.filter((link) => link.engine === "linux").slice(0, 1),
  ].filter(Boolean);
  const queryText = visibleLinks.map((link) => `${link.label}:\n${link.query}`).join("\n\n");
  const linkButtons = visibleLinks
    .map((link) => {
...
function bindEvents() {
  ...
  els.toggleAdvancedBtn.addEventListener("click", () => {
    ...
    els.toggleAdvancedBtn.textContent = hidden ? "高级筛选" : "收起筛选";
  });
